...
import (
	gorm2 "dora/app/manage/datasource/gorm"
...
func (u *UserResource) EmailCodeLogin(c *gin.Context) {
	...
	err := gorm2.Instance().First(&user, entity.User{Email: p.Email}).Error
...
func (u *UserResource) Captcha(c *gin.Context) {
	...
	db := gorm2.Instance().First(&user, entity.User{
		Email: p.Email,
	})
...
func (u *UserResource) UpdateSession(c *gin.Context) {
	...
	// 去数据库查最新的
	var user entity.User
	first := gorm2.Instance().First(&user, entity.User{
...
func (u *UserResource) UpdateInfo(c *gin.Context) {
	...
	// 更新
	err := gorm2.Instance().Model(&user).Updates(entity.User{
		NickName: p.NickName,
		Email:    p.Email,
		Password: p.Password,
	}).Error
	...
	if userCache.(entity.User).ID == p.Id {
		session.Set(userSessionKey, user)
		session.Options(sessions.Options{
			Path:     "/",
			MaxAge:   3600 * 1, // 1hrs
...
func (u *UserResource) List(c *gin.Context) {
	...
	db := gorm2.Instance().Limit(size).Offset((current - 1) * size)
	var err error
	cond := "%" + p.SearchStr + "%"
	logx.Printf("%v\n", cond)
