...
func (i *issues) CornUpdateCheck() {
	ctx := context.Background()
	keys, err := redis.Instance().SMembers(ctx, constant.Md5ListWaitUpdate).Result()
	if err != nil {
		logx.Error(err)
		return
	}
...
func (i *issues) CreateIssues(slsLog map[string]string) {
	...
	// 添加 md5 到已有列表
	redis.SetAdd(constant.Md5ListHas, slsLog["agg_md5"])

	// 删除这个 md5
	ctx := context.Background()
	_, err = redis.Instance().SRem(ctx, constant.Md5ListWaitCreate, slsLog["agg_md5"]).Result()
	if err != nil {
		logx.Error(err)
		return
	}
...
func (i *issues) UpdateIssues(md5 string) {
	...
	// 清空
	ctx := context.Background()
	_, err = redis.Instance().Del(ctx, constant.Md5ListWaitUpdate).Result()
	if err != nil {
		logx.Error(err)
		return
	}
...
func (i *issues) GetAllMd5() []string {
	var md5s []string
	err := gorm.Instance().Model(&entity.Issue{}).Select("md5").Find(&md5s).Error
	if err != nil {
		return nil
	}
	return md5s
